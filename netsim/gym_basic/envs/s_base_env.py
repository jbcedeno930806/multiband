...
class S_BASE_ENV(gym.Env, ABC):
    ...
    def state(self):
        ...
        con2 = Connection(
            self.connection.eventID,
            self.connection.src,
            self.connection.dst,
            self.connection.bitRate,
        )
        status, con2 = self.sap_allocator(con2, self.simulator.network, 0)
        if status == AllocationResult.Allocated:
            self.connection.protected = True
            self.simulator.network.allocate(con2)
            self.event.setType(EventType.Departure)
            self.simulator.eventsGenerator.appendEvent(self.event)
        return self._state(self.connection, con2.routeIndex)
